...
import { sendToDiscord } from '../../integrations/discord/send/route';
...
function isTargetBranch(ref?: string): boolean {
  ...
  return branchName === 'main' || branchName === 'develop' || branchName === 'master';
}

// ฟังก์ชันสร้าง Discord Embed สำหรับส่งไปยัง Discord
import { sendDiscordEmbed } from '../../integrations/discord/send/route';
...
function createDiscordEmbed(payload: GitHubPayload) {
  ...
  const commit = head_commit || (commits && commits.length > 0 ? commits[0] : null);
  ...
  if (commit) {
    fields.push(
      ...
      {
        name: '⏰ เวลา',
        value: commit.timestamp ? new Date(commit.timestamp).toLocaleString('th-TH') : 'ไม่ระบุ',
        inline: false
      }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('ได้รับ GitHub webhook payload:', JSON.stringify(payload, null, 2));
    
    // ตรวจสอบว่าเป็น branch ที่ต้องการหรือไม่
    if (!isTargetBranch(payload.ref)) {
      console.log(`ไม่ใช่ branch ที่ต้องการ: ${payload.ref}`);
      return NextResponse.json({ message: 'ไม่ใช่ branch ที่ต้องการ (main หรือ develop)' }, { status: 200 });
    }
    ...
    console.log('กำลังส่งข้อความไปยัง Discord:', JSON.stringify(embeds, null, 2));
    ...
    try {
      ...
      if (repository && sender && commit) {
        ...
        const deploymentResponse = await fetch('http://localhost:3000/api/deployments', {
          method: 'POST',
        ...
        if (!deploymentResponse.ok) {
          throw new Error(`การบันทึกข้อมูล deployment ล้มเหลว: ${deploymentResponse.status}`);
        }
        
        console.log('บันทึกข้อมูล deployment สำเร็จ');
      }
    } catch (deploymentError) {
      console.error('เกิดข้อผิดพลาดในการบันทึกข้อมูล deployment:', deploymentError);
      // ไม่ return error เพื่อให้การส่งข้อความไปยัง Discord สำเร็จ
    }
    
    return NextResponse.json({ success: true, message: 'ส่งข้อความไปยัง Discord สำเร็จ' });
  } catch (error) {

...
function analyzeCode(
  ...
  console.log(codeSnippet, repositoryUrl, branch);
  ...
  const suggestions = [
    {
      ...
      message:
        "ควรใช้ TypeScript interface แทนการใช้ type ในกรณีนี้เพื่อให้สามารถ extend ได้ในอนาคต",
      severity: "info" as const,
    },
    {
      category: "Documentation",
      message:
        "ควรเพิ่ม JSDoc comment สำหรับฟังก์ชันหลักเพื่อให้เข้าใจการทำงานได้ง่ายขึ้น",
      severity: "warning" as const,
    },
    {
      category: "Performance",
      message:
        "ควรใช้ useMemo หรือ useCallback เพื่อป้องกันการ re-render ที่ไม่จำเป็น",
      severity: "info" as const,
    },
  ];
  ...
  if (overall >= 90) {
    ...
    feedback =
      "โค้ดใช้งานได้ แต่ควรปรับปรุงในหลายส่วนเพื่อให้เป็นไปตาม best practices มากขึ้น";
  } else {
    feedback =
      "โค้ดควรได้รับการปรับปรุงอย่างมาก โดยเฉพาะในด้านการทำตาม best practices และประสิทธิภาพ";
  }
...
export async function POST(request: NextRequest) {
  try {
    const {
      ...
      repositoryUrl,
      branch,
    }: CodeAnalysisRequest = await request.json();

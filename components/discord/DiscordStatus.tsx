...
const DiscordStatus = ({ className = '' }: DiscordStatusProps) => {
  ...
  const validateDiscordConfig = async () => {
    ...
    try {
      ...
      if (!discordId || !discordToken || 
        ...
        setStatus('error');
        setMessage('ยังไม่ได้ตั้งค่า Discord webhook กรุณาตั้งค่าใน .env.local');
        return;
      }
      ...
      if (data.success) {
        setStatus('success');
        setMessage('การตั้งค่า Discord webhook ถูกต้อง พร้อมสำหรับการแจ้งเตือน');
      } else {
        throw new Error(data.message || 'ไม่สามารถเชื่อมต่อกับ Discord webhook ได้');
      }
    } catch (error) {
      setStatus('error');
      setMessage(`เกิดข้อผิดพลาด: ${error instanceof Error ? error.message : 'ไม่สามารถเชื่อมต่อกับ Discord webhook ได้'}`);
    }
  };
  ...
  const testDiscordMessage = async () => {
    ...
    try {
      ...
      if (data.success) {
        setStatus('success');
        setMessage('เชื่อมต่อกับ Discord webhook สำเร็จ กรุณาตรวจสอบข้อความใน Discord channel');
        setHasTestedThisSession(true);
      } else {
        throw new Error(data.message || 'เกิดข้อผิดพลาดในการเชื่อมต่อกับ Discord webhook');
      }
    } catch (error) {
      setStatus('error');
      setMessage(`เกิดข้อผิดพลาดในการส่งข้อความทดสอบ: ${error instanceof Error ? error.message : 'ไม่สามารถส่งข้อความไปยัง Discord ได้'}`);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  return (
    <div className={`relative rounded-xl shadow-md overflow-hidden transition-all duration-300 ${className} ${currentStyle.container} border`}>
      {/* Discord Icon Badge */}
      ...
      <div className="px-6 py-4 border-b border-opacity-30 border-gray-300">
        <div className="flex items-center">
          <div className={`flex items-center justify-center w-10 h-10 rounded-full mr-4 shadow-inner ${currentStyle.icon}`}>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${currentStyle.badge}`}>
                {status === 'loading' ? 'กำลังตรวจสอบ' : 
                 status === 'success' ? 'พร้อมใช้งาน' : 
                 'ไม่พร้อมใช้งาน'}
              </span>
            </div>
      ...
      <div className="p-6">
        {message && (
          <div className={`mb-4 text-sm ${currentStyle.message} bg-white bg-opacity-60 rounded-lg p-3 shadow-sm`}>
            {message}
          </div>
        )}
